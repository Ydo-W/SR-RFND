...
class BandGapDataset(Dataset):
    def __init__(self, para, data_type, mask=None, isNormalize=True):
        if mask is None:
            mask = [i for i in range(7)]
        mask = mask + [7]
        self.data_type = data_type

        # -------------------- 数据读取 ----------------------------------
        train_file_path = para.data_root + 'train_selected.txt'
        valid_file_path = para.data_root + 'valid_selected.txt'
        self.train_df = np.loadtxt(train_file_path)[:, mask]
        self.valid_df = np.loadtxt(valid_file_path)[:, mask]

        # -------------------- 数据划分 ----------------------------------
        self.train_num = self.train_df.shape[0]
        self.valid_num = self.valid_df.shape[0]
        ...
        if isNormalize:
            ...
            self.train_df = (self.train_df - min_) / (max_ - min_ + 1e-9)
            self.valid_df = (self.valid_df - min_) / (max_ - min_ + 1e-9)
